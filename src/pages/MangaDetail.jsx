    ...
    return (
        <div className="  p-4 md:p-8 flex flex-col items-center">
            ...
            <div className='flex items-center font-poppins px-4 py-2'>
                <div className='text-xs mr-16 text-[#EF8481] bg-[#FFE0DF] rounded-2xl h-[37px] w-[70px] justify-center flex items-center  font-poppins'>{manga?.category_id?.name}</div>
                <h2 className=' text-xl tracking-wider text-[#9D9D9D] '>{manga.author_id?.name.toUpperCase()}</h2>
            </div>
            <div className="flex items-center mt-2">
                <span> <img src={iconGood} alt="" /></span>
                <span> <img src={iconBad} alt="" /></span>
                <span> <img src={iconOpenVoice} alt="" /></span>
                <span> <img src={iconLove} alt="" /></span>
            </div>
            ...
            <div className="w-52 flex items-center cursor-pointer rounded-full h-8 bg-white border border-gray-400">
                <input
                    type="button"
                    value="Manga"
                    onClick={() => { setShow(!show), setTitle(!title) }}
                    className={`flex items-center justify-center w-1/2 h-full rounded-full text-[#9D9D9D] ${title ? 'bg-orange-500 text-white' : 'bg-[#EBEBEB] text-[#9D9D9D]'}`}
                />
                <input
                    type="button"
                    value="Chapter"
